...
def collect_episode_data(env, env_name: str, episode_id: int, ignore_quit_for: int, base_dir=None):
    ...
    while True:
        # Debug: Check what cameras are available (only once)
        if step_count == 0:
            print(f"Available cameras: {list(obs['image'].keys())}")
            print(f"Robot UID: {env.robot_uid}")
        
        # Use the specific render camera that matches manual control script
        # Access the render camera directly from the environment's camera system
        if hasattr(env.unwrapped, '_cameras') and 'render_camera' in env.unwrapped._cameras:
            render_camera = env.unwrapped._cameras['render_camera']
            render_frame = render_camera.take_picture()["rgb"]
        else:
            # Fallback to env.render() if render camera not available
            render_frame = env.render()
        
        img = Image.fromarray(render_frame)
        ...
        if step_count > 0 and step_count % 5 == 0 and base_dir is not None:
            ...
            render_frames_dir = episode_frames_dir / "render_camera"
            ...
            # Save render camera frame (what you see during demo)
...
def collect_trajectory(env_name: str, num_trajs: int):
    ...
    # Set up Jaco-specific camera configuration (matches demo_manual_control_custom_envs.py)
    ...
    env_kwargs = {
        ...
        "render_mode": "cameras",  # Keep this for now
        "robot": "jaco",
        "sim_freq": 501,
        "control_freq": 3,
        "scene_name": "google_pick_coke_can_1_v4",
        "render_camera_cfgs": render_camera_cfgs,
    ...
    if frames_dir.exists():
        ...
        render_files = list(frames_dir.glob("*/render_camera/*.png"))
        ...
        print(f"   Render camera frames: {len(render_files)}")
